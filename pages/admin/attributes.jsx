...
import { addAttribute, deleteData, editAddtribute, getAttributes } from '@/functions/globalFunctions'
...
function Attributes() {
    ...
    useEffect(() => {
        (async () => {
            try {
                ...
                let response = await getAttributes(obj)
                if (response) {
                    setData(response)
                }
            } catch (err) {
    ...
    const handleDelete = async (id, type) => {
        const token = sessionStorage.getItem('token');
        const user_id = sessionStorage.getItem('userid');
        var obj = { token: token, userid: user_id, ids: id, action: 'deleteAttributes' }
    ...
    const handleEdit = (id, data) => {
        ...
        setDetailObj({
            ...
            action: (prop) => { editAddtribute(prop) },
            data: data
        })
    }
    ...
    return (
        <Layout page={'Attributes'}>
            <div className='px-3 text-end'>
                <button onClick={() => handleAdd()} className='btn btn-primary'>Add Attribute</button>
