...
import { editBrand, deleteData, addBrand, getBrands } from '@/functions/globalFunctions'
...
function Brands() {
    ...
    useEffect(() => {
        (async () => {
            try {
                ...
                let response = await getBrands(obj)
                if (response) {
                    setData(response)
                }
            } catch (err) {
    ...
    const handleDelete = async (id, type) => {
        const token = sessionStorage.getItem('token');
        const user_id = sessionStorage.getItem('userid');
        var obj = { token: token, userid: user_id, ids: id, action: 'deleteBrand' }
    ...
    return (
        <Layout page={'Brands'}>
            <div className='px-3 text-end'>
                <button onClick={() => handleAdd()} className='btn btn-primary'>Add Brands</button>
