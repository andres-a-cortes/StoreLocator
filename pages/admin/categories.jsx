...
import { addCategory, deleteData, editCategory, getCategories } from '@/functions/globalFunctions'
...
function Categories() {
    ...
    const inputs = [
        ...
        { label: 'Icon', key: 'icon', type: 'text', required: true },
        { label: 'Parent', key: 'parent', type: 'text', required: true }
    ]
    ...
    useEffect(() => {
        (async () => {
            try {
                ...
                let response = await getCategories(obj)
                if (response) {
                    setData(response)
                }
            } catch (err) {
    ...
    const handleDelete = async (id, type) => {
        const token = sessionStorage.getItem('token');
        const user_id = sessionStorage.getItem('userid');
        var obj = { token: token, userid: user_id, ids: id, action: 'deleteCategories' }
        ...
        <Layout page={'Categories'}>
            <div>
                <div className='px-3 text-end'>
                    <button onClick={() => handleAdd()} className='btn btn-primary'>Add Category</button>
                    {ids?.length > 0 && <button className='btn btn-danger ms-2' onClick={() => handleConfirmation(ids.join(','), 'multi')}>Delete Selected</button>}
                </div>
                <div className="divTable">
                    <TableDataComp data={data} thead={thead} pageHeading={'Active Subscriptions'} loading={loading} />
                </div>
            </div>
            {editModal && <EditData isOpen={editModal} toggle={() => setEditModal(!editModal)} obj={detailObj} text={'Add Category'} />}
